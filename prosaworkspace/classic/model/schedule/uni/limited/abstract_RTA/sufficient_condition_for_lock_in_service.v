...
Module AbstractRTALockInService. 
  ...
  Section LockInService.
    ...
    Section InterferenceIsComplement.
      ...
      Proof. 
        have WORK := H_work_conserving.
        rewrite /work_conserving in WORK.
        ...
        - rewrite [X in X <= _]big_nat_cond [X in _ <= X]big_nat_cond.
          ...
          have WC := WORK j t1 t2 x H_j_arrives H_job_of_tsk H_job_cost_positive H_busy_interval.
          have RANGE: t1 <= x < t2.
          { apply/andP; split.
            - by apply leq_trans with t.
            - by apply leq_trans with (t + delta).
          }
          move: (WC RANGE) => [WC1 WC2].
          have NOTINT: ~ interference j x.
          { apply WC2.
            rewrite lt0b in CONTR.
            exact CONTR. }
          by rewrite IJX in NOTINT.
        - rewrite [X in X <= _]big_nat_cond [X in _ <= X]big_nat_cond.
          apply leq_sum; move => x /andP [/andP [GE2 LT2] _ ].
          ...
          { apply/andP; split.
            ...
            - by apply leq_trans with (t + delta). }
          move: (WC RANGE) => [WC1 WC2].
          have SCHED: scheduled_at sched j x.
          { apply WC1.
            by rewrite IJX. }
          by move: SCHED; rewrite lt0b.
      Qed.
    ...
    Section InterferenceBoundedImpliesEnoughService.
      ...
      Proof.
        ...
        { intros.
          ...
          rewrite /service.
          rewrite (service_during_cat _ _ t2); last first.
          { apply/andP; split.
            - by done.
            - apply negbT in NEQ.
              by apply ltnW; rewrite ltnNge.
          }
          apply leq_trans with (service_during sched j 0 t2); [by done | by rewrite leq_addr].
        } 
        { have BOUND:
            progress_of_job <= delta - cumul_interference j t1 (t1 + delta).
          { move: H_total_workload_is_bounded.
            ssromega.
          }
          ...
          { unfold service.
            rewrite [X in _ <= X](service_during_cat _ _ t1); last first.
            { apply/andP; split.
              - by done.
              - by rewrite leq_addr.
            }
