    ...
    Section DecidableQuietTime.
      ...
      Proof.
        ...
        { intros QT s ARRs HPs BEFs.
          move: QT => /allP QT.
          have INs: s \in jobs_arrived_before arr_seq t.
          {
            eapply arrived_between_implies_in_arrivals; eauto 2.
          }
          move: (QT s INs) => /implyP Q.
          by apply Q in HPs.
        }
      ...
      (* Then, we prove that there is a job pending at time t2
        ...
        Proof.
          ...
          }
          {
            ...
            have INarr: j_hp \in arrivals_between t1 t2.
            {
              eapply arrived_between_implies_in_arrivals; eauto 1.
              by apply/andP; split.
            }
            move: (COMP j_hp INarr).
            by rewrite /= HP andbT negbK.
          }
        Qed.
      ...
      Section ProcessorAlwaysBusy.
        ...
        Proof.
          ...
          { move: (H_busy_interval_prefix) => [_ [QT [NQT _]]].
            ...
            move: EX => [hps SE].
            destruct hps as [|jhp jobs] eqn:FL.
              { exfalso.
                move: GE; rewrite leq_eqVlt => /orP [/eqP EQ|GE].
                - subst t.
                  have NOTQT : ~ quiet_time t1.+1.
                  { apply NQT.
                    by apply/andP; split. }
                  apply NOTQT.
                  ...
                  have CONTR: jhp \in [::].
                  {
                    apply SE2. repeat split; try done.
                    apply/andP; split.
                    + by rewrite /arrived_before /has_arrived in ARRB; rewrite ltnS in ARRB.
                    + apply/negP; intros COMLP.
                      move: NCOMP => /negP NCOMP.
                      apply: NCOMP.
                      by apply completion_monotonic with t1.
                  }
                  by case: CONTR.
                - have NOTQT : ~ quiet_time t.
                  { apply NQT.
                    by apply/andP; split. }
                  apply NOTQT.
                  intros jhp ARR HP ARRB.
        ...
        Proof.
          ...
          have BACK: backlogged job_arrival job_cost sched jhp t.
          {
            apply/andP; split; first exact PEND.
            move: IDLE => /eqP IDLE. unfold scheduled_at. rewrite IDLE. by done.
          }
          have WC := H_work_conserving _ t ARR BACK.
          move: WC => [jo SCHED].
      ...
      Section QuietTimeAndServiceOfJobs.
        ...
        Proof.
          ...
          { rewrite [in X in X <= _]big_nat_cond [in X in _ <= X]big_nat_cond //=; rewrite leq_sum //.
            ...
            change (0 < \sum_(j <- arrivals_between 0 (t1 + Δ) | true) service_at sched j t').
            rewrite sum_nat_gt0 filter_predT.
            apply/hasP.
            ...
            { exists j1.
              - have ARRj1 : arrives_in arr_seq j1.
                { apply (H_jobs_come_from_arrival_sequence j1 t').
                  rewrite /scheduled_at SCHED; by done. }
                eapply arrived_between_implies_in_arrivals.
                + exact H_arrival_times_are_consistent.
                + exact ARRj1.
                + apply/andP; split; first by done.
          ...
          Section LowerBound.
            ...
            Proof. 
              ...
              }
              {
                ...
                - by apply/andP; split; last by move: PEND => /andP [ARR _].
              }
            Qed.             
            ...
            Section CannotBeBusyForSoLong.
              ...
              Proof. 
                ...
                { rewrite /hp_service hep_jobs_receive_no_service_before_quiet_time //.
                  rewrite /service_of_higher_or_equal_priority_jobs /service_of_jobs.
                  have H := no_idle_time_within_non_quiet_time_interval
                              H_arrival_sequence_is_a_set
                              H_jobs_must_arrive_to_execute
                              H_work_conserving
                              t1 delta
                              H_no_quiet_time.
                  unfold service_of_jobs in H.
                  rewrite -[X in X <= _]H.
                  rewrite big_mkcond (bigID (fun j' => higher_eq_priority j' j)) /=.
                  set hp := \sum_(j' <- arrivals_between 0 (t1 + delta) | higher_eq_priority j' j)
                                 service_during sched j' t1 (t1 + delta).
                  set lp := \sum_(j' <- arrivals_between 0 (t1 + delta) | ~~ higher_eq_priority j' j)
                                 service_during sched j' t1 (t1 + delta).
                  have LE_nonhp :
                    lp <= cumulative_priority_inversion j t1 (t1 + delta).
                  { rewrite /lp /cumulative_priority_inversion /is_priority_inversion exchange_big //=.
                  ...
                  have Hsum : lp + hp <= cumulative_priority_inversion j t1 (t1 + delta) + hp.
                  { apply: leq_add.
                    - exact LE_nonhp.
                    - exact (leqnn hp).
                  }
                  have Hswap : hp + lp = lp + hp by rewrite addnC.
                  rewrite Hswap.
                  exact Hsum.
                }
                { rewrite leq_add2r.
              ...
              Proof.
                ...
                rewrite -(UniprocessorSchedule.ignore_service_before_arrival job_arrival sched H_jobs_must_arrive_to_execute j0 0 (t1 + delta));
                  rewrite //; [| by apply ltnW].
              ...
              { have EX': exists (t2: nat), ((t1 < t2 <= t1 + delta) && quiet_time_dec j t2).
                ...
                - move => t /andP [GT1 LT2] BUG.
                  have MINt : t2 <= t.
                  {
                    ...
                    - apply/andP; split.
                      + exact GT1.
                      + by apply leq_trans with (n := t2); [by apply ltnW |].
                  ...
                  by apply leq_ltn_trans with (p := t2) in MINt; first by rewrite ltnn in MINt.
                - {
                  ...
                  have NEQ: t_busy < t2.
                  {
                    ...
                    move: QUIET => /allP QUIET.
                    have INjhp : jhp \in arrivals_between 0 t2.
                    { by eapply arrived_between_implies_in_arrivals; eauto 2. }
                    move: (QUIET jhp INjhp) => /implyP QUIETjhp.
                    by apply QUIETjhp.
                  }
