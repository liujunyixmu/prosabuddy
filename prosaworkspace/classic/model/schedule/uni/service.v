...
Require Import prosa.util.sum prosa.util.unit_growth.
...
Module Service.
  ...
  Section ExtraLemmas.
    ...
    Proof.
      ...
      { case SCHED: (scheduled_at sched j t1).
        ...
        { apply negbT in SCHED.
          move: SERV; rewrite /service /service_during sum_nat_gt0; move => /hasP [t]; rewrite mem_filter => /andP [_ IN] SCHEDt.
      ...
      { feed IHk; first by apply ltn_trans with k.+1.
        ...
        { rewrite (service_during_cat _ t); last by apply/andP; split.
        ...
        { apply negbT in SCHED.
          move: SERV; rewrite /service /service_during sum_nat_gt0; move => /hasP [x]; rewrite mem_filter => /andP [_ INx] SCHEDx.
      ...
      { case ARR: (j \in arrivals_between t1 t2).
        { rewrite (big_rem j) //=; simpl.
          ...
          - rewrite leqn0 big1_seq; first by done.
            ...
            rewrite rem_filter in ARRj'; last by eapply arrivals_uniq; eauto 2.
    ...
    Proof.
      ...
      have L' := L s; clear L; rename L' into L; feed L.
    ...
    Section WorkloadServiceAndCompletion. 
      ...
      Proof.
        ...
        { unfold workload_of_jobs, service_of_jobs in H.
          ...
          have ALLEQ: all (fun x => service_during sched x t1 t_compl == job_cost x)
                          [seq x <- arrivals_between t1 t2 | P x].
          { rewrite -eq_sum_leq_seq; last by intros; apply cumulative_service_le_job_cost.
            by rewrite -H eq_refl. }
          move: ALLEQ => /allP ALLEQ.
          have := ALLEQ j; rewrite mem_filter H1 ARR //=.
          move => /(_ isT) /eqP EQj.
          by rewrite /service_during in EQj; rewrite EQj.
        } 
      Qed.
