...
Module NonpreemptiveSchedule.
  ...
  Section Definitions.
    ...
    Section Lemmas.
      ...
      (* Helper lemma for arithmetic with natural number subtraction. *)
      Lemma subh3 : forall m n p, m + p <= n -> m <= n - p.
      ...
      Section CompletionUnderNonpreemptive.
        ...
        Proof.
          ...
          apply completion_monotonic with (t := i); last by done.
          by rewrite subn1 -ltnS prednK //; apply leq_ltn_trans with i.
        Qed.
        ...
        Section LeftBound.
              ...
              rewrite (@big_cat_nat _ _ _ ((t - service sched j t).+1)) //=.
              {
                ...
                apply leq_ltn_trans with
                 ...
                 { set (t - service sched j t).+1 as T.
                   ...
                   { apply leq_sum => i _; by destruct (scheduled_at sched j i). }
                   { simpl_sum_const. by done. }
                 }
                 { unfold job_remaining_cost, remaining_cost.
                   have SVC_LT_JC: service sched j t < job_cost j.
                   { rewrite ltnNge.
                     by apply scheduled_implies_not_completed. }
                   have SVC_LE: service sched j t <= t.
                   { have := CUMLED. by rewrite /service_during add0n. }
                   rewrite addnBA; [| by done].
                   rewrite -subnDA addnS subnS addnC subnKC; [| by done].
                   by rewrite addnK prednK. }
              }
              (* big_cat_nat side condition *)
              apply leq_ltn_trans with t.
              - by apply leq_subr.
              - rewrite -[X in X < _]addn0 ltn_add2l.
                rewrite /job_remaining_cost /remaining_cost subn_gt0.
                rewrite ltnNge.
                by apply scheduled_implies_not_completed.
            }
            {
              ...
              feed_n 4 Y; try(done).
              { apply/andP; split; [by rewrite ltnW | by rewrite leq_subr]. }
          ...
          Proof.
              ...
              t - service sched j t - 1 + job_remaining_cost j (t - service sched j t - 1).
            {
              ...
              rewrite -subh1; last by rewrite cumulative_service_le_delta.
              by rewrite -subh1; last first.
            }
