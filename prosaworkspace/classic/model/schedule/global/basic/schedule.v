  ...
  Section JobLemmas.
    ...
    Section Basic.
      ...
      Proof.
        ...
        {
          intros NOSERV; rewrite sum_nat_eq0_nat in NOSERV.
          move: NOSERV => /allP ALL.
          rewrite negb_exists; apply/forallP; intros cpu.
          apply/negP => SCHED_ON.
          have IN: cpu \in [seq x <- index_enum _ | sched x t == Some j].
          { by rewrite mem_filter SCHED_ON mem_index_enum. }
          by move: (ALL cpu IN).
        }
      Qed.
      ...
      Proof.
        ...
        destruct (scheduled sched j t) eqn:SCHED; unfold scheduled in SCHED.
        {
          ...
          rewrite -big_filter (bigD1_seq cpu);
            [simpl | | by rewrite deprecated_filter_index_enum enum_uniq];
