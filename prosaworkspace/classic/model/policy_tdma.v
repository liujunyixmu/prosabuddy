  ...
  Section PropertiesTDMA.
    ...
    Section TimeSlot.
      ...
      Section BasicLemmas.
        ...
        - rewrite -big_mkcond. rewrite-> bigD1_seq with (j:=task);
            [| by trivial | by apply (set_uniq ts)].
          rewrite -subn_gt0 -addnBA; [by rewrite subnn addn0 | by trivial].
        Qed.
        ...
        rewrite addnC (bigD1_seq task) //=; [| by apply (set_uniq ts)].
        rewrite leq_add2l.
        rewrite big_mkcond.
    ...
    Section InTimeSlotUniq.
      ...
      { symmetry. rewrite big_mkcond /=. rewrite->bigD1_seq with (j:=tsk1);auto;last by apply (set_uniq ts).
        ...
        have TS2: (tsk1 != tsk2) = true by apply /eqP;auto. rewrite TS2.
      ...
      fold O1 O2 in order;try ssromega;auto. { by move/eqP in NEQ. } { by apply /eqP;auto. }
      Qed.
