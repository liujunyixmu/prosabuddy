...
Module ConcreteSchedulerTDMA.
  ...
  ohead [seq job <- pending_jobs | job_in_time_slot job].
    ...
    Section SchedulerTDMA.
        ...
        have EQ_pending: pending_jobs sched1 t = pending_jobs sched2 t.
  {
    rewrite /pending_jobs.
    apply eq_in_filter => job _.
    rewrite /pending.
    have EQ_service: service sched1 job t = service sched2 job t.
  {
    rewrite /service.
    apply eq_big_nat => t0 /andP [_ Hlt].
    rewrite /service_at /scheduled_at.
    by rewrite (ALL t0 Hlt).
  }
  by rewrite /completed_by EQ_service.
  }
  rewrite EQ_pending.
  by [].
    ...
    Proof.
     ...
     move: SCHED.
      case F: [seq job <- pending_jobs scheduler_tdma t | Task_in_time_slot ts slot_order (job_task job) time_slot t] => [|x xs] //=.
      case=> EQ. 
      have PEN: j \in pending_jobs scheduler_tdma t.
      { 
        rewrite -EQ. 
    ...
    Proof.
      ...
      case=> EQ1.
      have PEN: j \in pending_jobs scheduler_tdma t.
      { rewrite -EQ1. have: x \in x :: xs by rewrite inE eq_refl. rewrite -F mem_filter. by case/andP. }
      have IN: Task_in_time_slot ts slot_order (job_task j) time_slot t.
      { rewrite -EQ1. have: x \in x :: xs by rewrite inE eq_refl. rewrite -F mem_filter. by case/andP. }
