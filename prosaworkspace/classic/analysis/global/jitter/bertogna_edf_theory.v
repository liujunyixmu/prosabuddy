  ...
  Section ResponseTimeBound.
    ...
    Section Lemmas.
      ...
      Section LemmasAboutInterferingTasks.
        ...
        Proof.
          have H_tsk_in_ts: tsk \in ts.
{
  have H_in: tsk \in unzip1 rt_bounds by apply/map_f: H_tsk_R_in_rt_bounds.
 
  by rewrite H_rt_bounds_contains_all_tasks in H_in. }


have H_tsk_other_in_ts: tsk_other \in ts.
{ have H_other_in: tsk_other \in unzip1 rt_bounds by apply/map_f: H_response_time_of_tsk_other.
  by rewrite H_rt_bounds_contains_all_tasks in H_other_in. }

apply interference_bound_edf_bounds_interference with (job_deadline := job_deadline) 
                                                      (arr_seq := arr_seq) 
                                                      (ts := ts) => //.
- exact: H_tasks_miss_no_deadlines.
- apply: (leq_trans _ (H_tasks_miss_no_deadlines tsk R H_tsk_R_in_rt_bounds)).
  lia.
    -move=> j_k H_jk_arrives H_jk_task H_jk_time_lt.
exact: H_all_previous_jobs_completed_on_time j_k tsk_other R_other H_jk_arrives H_jk_task H_response_time_of_tsk_other H_jk_time_lt.      
        Qed.
    ...
    Section MainProof.
      ...
      Proof.
        ...
        specialize (EDFBOUND tsk' R' INbounds j ARRj JOBtsk NOTCOMP BEFOREok tsk_other R_other HP).
