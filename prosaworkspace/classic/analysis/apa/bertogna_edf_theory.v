  ...
  Section ResponseTimeBound.
    ...
    Section Lemmas.
      ...
      Section LemmasAboutInterferingTasks.
        ...
        Proof.
 have TSK_IN : tsk \in ts.
  { by rewrite -H_job_of_tsk; apply: H_all_jobs_from_taskset. }
  
  apply interference_bound_edf_bounds_interference with (job_deadline := job_deadline)
                                                       (arr_seq := arr_seq) (ts := ts); try (by done).
  - apply: (leq_trans (H_at_least_one_cpu tsk TSK_IN)).
    unfold affinity in *.
    apply: (leq_trans (max_card _)).
    by rewrite card_ord.
  - have H_in_unzip : tsk_other \in unzip1 rt_bounds.
{ by apply/mapP; exists (tsk_other, R_other). }
by rewrite H_rt_bounds_contains_all_tasks in H_in_unzip.
  - exact: (H_tasks_miss_no_deadlines tsk_other R_other H_response_time_of_tsk_other).
  - exact: (H_tasks_miss_no_deadlines tsk R H_tsk_R_in_rt_bounds).
