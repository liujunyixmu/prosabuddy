...
(** * RTA for EDF Scheduling with Floating Non-Preemptive Regions on Uniprocessors under the Average Resource Model *)
...
(** In the following, we derive a response-time analysis for EDF schedulers,
    ...
    uniprocessor under the average resource model (inspired by the paper
    "Periodic Resource Model for Compositional Real-Time Guarantees" by Shin &
    Lee, RTSS 2003). To this end, we instantiate the sequential variant of
...
Section RTAforFloatingEDFModelwithArrivalCurves.
  ...
  (** Consider tasks characterized by a WCET [task_cost], relative deadline
      [task_deadline], an arrival curve [max_arrivals], and a bound on the
      task's longest non-preemptive segment [task_max_nonpreemptive_segment],
      ... *)
  ...
  (** Finally, using the sequential variant of abstract restricted-supply
      ...
      unit-speed uniprocessor under the average resource model. *)
