...
Section FixedPoint.
  ...
  Proof.
    ...
    suff H: forall x1 delta, iter x1 f x0 <= iter (x1 + delta) f x0.
    { by rewrite -(subnK LE) addnC; apply H. }
    intros x1' delta.
  ...
  Proof.
    ...
    suff H: forall x1 delta, le (iter x1 f x0) (iter (x1 + delta) f x0).
    { by rewrite -(subnK LE) addnC; apply H. }
    intros x1' delta.
