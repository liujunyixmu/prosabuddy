...
Section find_seq.
  ...
  Proof.
    ...
    - case (a==x)eqn:AX;case (a==y)eqn:AY;simpl; rewrite AX AY;auto.
      ...
      + simpl in FIND;move/eqP in AY;rewrite AY PY in FIND;clarify;
        by move/eqP in AX. 
      + rewrite in_cons in YIN. move:YIN=> /orP [/eqP EQ | YIN].
        * move/eqP in AY;auto.
        * simpl in FIND. case (P a) eqn:PA;clarify.
          -- move/eqP in AX;auto.
          -- apply IHl;auto.
  Qed.
  ...
  Lemma find_uniql:
    ...
    ~ x \in l1.
  Proof.
    intros x l1 l2 H H0. intro XIN.
    induction l1.
    + auto.
    + case (a==x)eqn:AX;move/eqP in AX;simpl in H;
      move:H=>/andP [NIN U];move/negP in NIN.
      - rewrite AX in NIN.
        exfalso; apply NIN; rewrite mem_cat; apply/orP; by right.
      - apply IHl1; auto.
        rewrite in_cons in XIN.
        move:XIN=>/orP [/eqP EQ | XL1]; [by rewrite EQ in AX | exact XL1].
  Qed.
    ...
    - simpl in G. case (P a) eqn:CASE.
      ...
      + case (a==x) eqn:AX.
        ...
        * rewrite in_cons in IN. move/orP in IN.
          ...
          -- destruct (IHl _ G XINL) as [NP|EXIST].
             ++ by left.
