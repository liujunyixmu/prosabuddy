...
export const EditTool = Tool.define("edit", {
  ...
  async execute(params, ctx) {
    ...
    ProofEditTransaction.assertStagedReadSynchronized(ctx.sessionID, filePath, "editing the proof")
    ...
    await FileTime.withLock(filePath, async () => {
      if (params.oldString === "") {
        ...
        proofWorkflowTakeover = SessionProofWorkflow.assertWideAgentRunningRegionEditAllowed({
          sessionID: ctx.sessionID,
          agent: ctx.agent,
          file: filePath,
          before: contentOld,
        ...
        if (proofTransaction && params.takeover_running_region && params.takeover_reason?.trim()) {
          ...
          SessionProofWorkflow.recordSourceMutation(filePath, contentNew)
        }
      ...
      contentOld = ProofEditTransaction.source(ctx.sessionID, filePath) ?? await Filesystem.readText(filePath)

      const ending = detectLineEnding(contentOld)
      const old = convertToLineEnding(normalizeLineEndings(params.oldString), ending)
      const next = convertToLineEnding(normalizeLineEndings(params.newString), ending)

      EditConflictGuard.assertAllowed({ sessionID: ctx.sessionID, file: filePath, source: contentOld })
      try {
        contentNew = replace(contentOld, old, next, params.replaceAll)
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error)
        if (message.includes("Could not find oldString") || message.includes("Found multiple matches for oldString")) {
          throw new Error(
    ...
    let output = proofTransaction
      ? `Edit staged in proof transaction ${proofTransaction.transaction_id}; the workspace file is unchanged until an accepted checkpoint is committed.`
