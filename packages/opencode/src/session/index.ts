...
export namespace Session {
  ...
  export async function createNext(input: {
    ...
    const result: Info = {
      ...
      workspaceID: WorkspaceContext.current().workspaceID,
      parentID: input.parentID,
  ...
  export function* list(input?: {
    ...
    const workspaceID = WorkspaceContext.current().workspaceID

    if (workspaceID) {
      conditions.push(eq(SessionTable.workspace_id, workspaceID))
    }
    if (input?.directory) {
  ...
  export const remove = fn(Identifier.schema("session"), async (sessionID) => {
    ...
    try {
      ...
      const { SessionProofWorkflow } = await import("./proof-workflow")
      const { SessionProof } = await import("./session-proof")
