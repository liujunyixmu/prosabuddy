...
export namespace Filesystem {
   ...
   * before rename. The previous file remains intact until the rename succeeds.
   */
  export async function writeAtomic(
    ...
    const rawMode = stat(p)?.mode
    const existingMode = typeof rawMode === "bigint" ? Number(rawMode) : rawMode
    const finalMode = mode ?? (existingMode === undefined ? undefined : existingMode & 0o777)
    const temporary = join(directory, `.${basename(p)}.${process.pid}.${randomUUID()}.tmp`)
    let handle: Awaited<ReturnType<typeof open>> | undefined
    try {
      handle = await open(temporary, "wx", finalMode ?? 0o666)
      ...
      // directory handle. Failure here does not undo an already atomic rename.
