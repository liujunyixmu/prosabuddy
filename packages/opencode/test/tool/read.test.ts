...
describe("tool.read proof transaction source", () => {
  test("reads a recovered staged revision instead of the stale workspace file", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        await ProofEditTransaction.begin({
          sessionID: "read-old-proof-session",
          parentSessionID: "",
          agent: "prover",
          file,
          source: diskSource,
        ...
        expect((await ProofEditTransaction.finalize("read-old-proof-session"))?.status).toBe("recoverable")

        const recovered = ProofEditTransaction.recoverLatest({
          sessionID: "read-fresh-proof-session",
          parentSessionID: "",
          agent: "prover",
          file,
          source: diskSource,
          theorem: "demo",
          minimumScope: { kind: "theorem_body", theorem: "demo" },
        })
        expect(recovered?.recovered).toBe(true)
        expect(ProofEditTransaction.requiresStagedRead("read-fresh-proof-session", file)).toBe(true)
        expect(() =>
          ProofEditTransaction.assertStagedReadSynchronized(
            "read-fresh-proof-session",
            file,
            "opening a Coq session",
          ),
        ).toThrow("proof_transaction_resync_required")
        ...
        ProofEditTransaction.abort("read-fresh-proof-session")
      },
    })
  })
})
...
describe("tool.read env file permissions", () => {
  ...
  describe.each(["prover", "whole-lemma"])("agent=%s", (agentName) => {
