...
describe("tool.assertExternalDirectory", () => {
  ...
  test("strict workspace boundary rejects external paths before asking", async () => {
    const before = process.env.OPENCODE_STRICT_WORKSPACE_BOUNDARY
    process.env.OPENCODE_STRICT_WORKSPACE_BOUNDARY = "1"
    const requests: Array<Omit<PermissionNext.Request, "id" | "sessionID" | "tool">> = []
    const ctx: Tool.Context = {
      ...baseCtx,
      ask: async (req) => {
      ...
      await Instance.provide({
        ...
        fn: async () => {
          await expect(assertExternalDirectory(ctx, "/tmp/outside/file.txt")).rejects.toThrow(
            "Path must stay within workspace: /tmp/project",
          )
        },
      })
    } finally {
      if (before === undefined) delete process.env.OPENCODE_STRICT_WORKSPACE_BOUNDARY
      else process.env.OPENCODE_STRICT_WORKSPACE_BOUNDARY = before
    }
