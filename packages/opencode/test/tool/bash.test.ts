...
describe("tool.bash", () => {
  ...
  test("kills commands whose combined output exceeds the hard byte limit", async () => {
    await using tmp = await tmpdir({ git: true })
    const previous = process.env.OPENCODE_BASH_MAX_OUTPUT_BYTES
    process.env.OPENCODE_BASH_MAX_OUTPUT_BYTES = "1024"
    try {
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          const result = await bash.execute(
            {
              command: "head -c 65536 /dev/zero | tr '\\0' x",
              description: "Generate bounded test output",
            },
            ctx,
          )
          expect(result.metadata.output_limit_exceeded).toBe(true)
          expect(result.metadata.output_bytes).toBeLessThanOrEqual(1024)
          expect(result.output).toContain("exceeded 1024 bytes")
        },
      })
    } finally {
      if (previous === undefined) delete process.env.OPENCODE_BASH_MAX_OUTPUT_BYTES
      else process.env.OPENCODE_BASH_MAX_OUTPUT_BYTES = previous
    }
  })
