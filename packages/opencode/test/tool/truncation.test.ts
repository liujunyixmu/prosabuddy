...
describe("Truncate", () => {
  describe("output", () => {
    ...
    test("writes full output to a caller-provided workspace directory", async () => {
      await using tmp = await tmpdir()
      const outputDirectory = path.join(tmp.path, ".prosabuddy-tool-output")
      const lines = Array.from({ length: 100 }, (_, i) => `line${i}`).join("\n")
      const result = await Truncate.output(lines, { maxLines: 10, outputDirectory })

      expect(result.truncated).toBe(true)
      if (!result.truncated) throw new Error("expected truncated")
      expect(path.dirname(result.outputPath)).toBe(outputDirectory)
      expect(result.content).toContain(result.outputPath)
      expect(await Filesystem.readText(result.outputPath)).toBe(lines)
    })
