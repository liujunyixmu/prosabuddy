...
describe("session.compaction proof history policy", () => {
  test("keeps medium proof history append-only to preserve the provider cache prefix", async () => {
    await using tmp = await tmpdir()
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        const model = createModel({ context: 1_000_000, output: 32_000 })
        const tokens = { input: 150_000, output: 10_000, reasoning: 0, cache: { read: 0, write: 0 } }
        expect(await SessionCompaction.isOverflow({ tokens, model })).toBe(false)
        expect(await SessionCompaction.shouldCompact({ tokens, model, agent: "lemma" })).toBe(false)
      },
    })
  })

  test("compacts a proof history once it reaches a high proportion of usable context", async () => {
    await using tmp = await tmpdir()
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        const model = createModel({ context: 1_000_000, output: 32_000 })
        const tokens = { input: 760_000, output: 0, reasoning: 0, cache: { read: 0, write: 0 } }
        expect(await SessionCompaction.isOverflow({ tokens, model })).toBe(false)
        expect(await SessionCompaction.shouldCompact({ tokens, model, agent: "prover" })).toBe(true)
        expect(await SessionCompaction.shouldCompact({ tokens, model, agent: "general" })).toBe(false)
      },
    })
  })

  test("still compacts proof history at the real provider boundary", async () => {
    await using tmp = await tmpdir()
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        ...
        expect(await SessionCompaction.shouldCompact({ tokens, model, agent: "lemma" })).toBe(true)
      },
    })
  })
})
